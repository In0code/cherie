					...
					<div style="margin: 10px; text-align: center;">
						...
						<table id="order_list" class="table tableList">
							...
							<c:if test="${ empty orderList }">
								<tr>
									<td colspan="11" style="text-align: center;">회원정보가 존재하지
										않습니다</td>
								</tr>
							</c:if>
							...
							<c:forEach var="order" items="${ orderList }" varStatus="i">
								...
								<tr>
									<td><input type="checkbox" class="check" name="check"
										id="check" value="${ order.ord_dno }"></td>
									...
									<td><select name="statuslist" id="statuslist">
											<option value="PF"
												${ order.dlvy_pro eq 'PF'? " selected='selected'" : "" }>결제완료
											</option>
											<option value="D0"
												${ order.dlvy_pro eq 'D0'? " selected='selected'" : "" }>배송중</option>
											<option value="DR"
												${ order.dlvy_pro eq 'DR'? " selected='selected'" : "" }>배송준비</option>
											<option value="DF"
												${ order.dlvy_pro eq 'DF'? " selected='selected'" : "" }>배송완료
											</option>
									</select></td>
									<td><c:out value="${ order.name }" /></td>
									<c:set var="totalAmount"
										value="${order.price * order.amount  + dlvyPrice}" />
